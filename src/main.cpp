...
int main(int argc, char **argv) {
  ...
  init_params.camera_disable_imu =
      true; // for this sample, IMU (of ZED-M) is disable, we use the gravity
            // given by the marker.
  ...
  cout << " Is the ARUCO Marker 6x6 ?, check "
       << actual_marker_size_meters * 1000 << " mm" << endl;
  ...
  while (key != 'q') {
    ...
    if (zed.grab() == SUCCESS) {
      ...
      zed.retrieveImage(image_zed, VIEW_LEFT, MEM_CPU, image_size.width,
                        image_size.height);
      ...
      cv::putText(
          image_ocv_rgb,
          "Loaded dictionary : 6x6.     Press 'SPACE' to reset the camera "
          "position",
      ...
      position_txt = "yaw zed: " + to_string(zed_rdy[2]) +
                     " ZED  x: " + to_string(zed_pose.pose_data.tx) +
                     "; y: " + to_string(zed_pose.pose_data.ty) +
                     "; z: " + to_string(zed_pose.pose_data.tz);

      cv::putText(image_ocv_rgb, position_txt, cv::Point(10, 35),
                  cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(236, 188, 26));
      ...
      if (ids.size() > 0) {
        ...
        cv::aruco::drawAxis(image_ocv_rgb, camera_matrix, dist_coeffs, rvecs[0],
                            tvecs[0], actual_marker_size_meters * 0.5f);

        // position_txt = "Aruco x: " + to_string(pose.tx) + "; y: " +
        // to_string(pose.ty) + "; z: " + to_string(pose.tz);

        int x = (corners[0][0].x + corners[0][1].x + corners[0][2].x +
                 corners[0][3].x) /
                4;
        int y = (corners[0][0].y + corners[0][1].y + corners[0][2].y +
                 corners[0][3].y) /
                4;
        ...
        position_txt = "distance z-axis: " + to_string(pose.tx * 100) +
                       " X: " + to_string(x) + " Y: " + to_string(y) +
                       " Distance is: " + to_string(distance);
      ...
      if (!lines.empty()) {
        ...
        // Plot lane detection
        //                lane_image = surroundEye.plotLane(image_ocv_rgb, lane,
        //                turn);

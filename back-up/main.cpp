...
for (int k = 0; k < 100; k++ ) {
    ...
    if (zed.grab() == sl::SUCCESS) { //1280x720
        ...
        zed.retrieveImage(depth_image, sl::VIEW_LEFT);
...
//                    std::cout << "Four Corners are " << markerCorners[i][j].x << typeid(markerCorners[i][j].x).name() << std::endl;
             ...
             x = int(x) / 4;  //calculating the moments or centroid
             y = int(y) / 4;
         ...
         // we are basically saying we have enough ids inside the vector of the markerIds
            ...
            std::cout << "Thjs raw value is " << x  <<  "and " << y << std::endl;
        ...
        cv::imshow( "ARUCO_RGB ", cv_RGB_image_copy);
